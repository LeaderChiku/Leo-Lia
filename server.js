...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!character || !messages || !Array.isArray(messages)) {
      return res.status(400).json({ error: 'Missing required parameters: character and messages.' });
    }

    // Determine the API Key to use: backend env key only (no client custom key)
    const apiKey = process.env.GEMINI_API_KEY;

    if (!apiKey) {
      return res.status(401).json({
        error: 'API_KEY_MISSING',
        message: 'Gemini API key is not configured. Please add it to the backend .env.'
      });
    }

    // Construct the customized system instruction based on the character, local time, and user preferences
...
- With others/General: Maintain a comforting, warm, cute, and highly caring best-friend vibe.`;
    ...
    // We use gemini-1.5-flash as it is free, fast, and perfect for text responses.
